...
class CourseDetailsController extends GetxController {
  ...
  // Calculate total learning time from video durations
  String get totalLearningTime {
    final syllabusData = syllabus;
    if (syllabusData == null || syllabusData.modules.isEmpty) return 'N/A';
    
    int totalMinutes = 0;
    int videosWithDuration = 0;
    
    for (final module in syllabusData.modules) {
      for (final topic in module.topics) {
        if (topic.videoDuration != null && topic.videoDuration!.isNotEmpty) {
          final duration = _parseDuration(topic.videoDuration!);
          if (duration > 0) {
    ...
    if (totalMinutes == 0) {
      // Fallback: estimate based on number of topics (assume 10 minutes per topic)
      final estimatedMinutes = syllabusData.totalTopics * 10;
      return _formatDuration(estimatedMinutes, isEstimate: true);
    }
    
    return _formatDuration(totalMinutes);
  }

  // Parse duration string to minutes (supports formats like "10:30", "5:45", "15", etc.)
  int _parseDuration(String duration) {
    try {
      duration = duration.trim();
      
      // Handle formats like "10:30" or "5:45"
      ...
      // Handle format like "15" (assume minutes)
  ...
  String _formatDuration(int totalMinutes, {bool isEstimate = false}) {
    if (totalMinutes < 60) {
      final estimate = isEstimate ? '~' : '';
      return '$estimate$totalMinutes min';
    } else if (totalMinutes < 1440) { // Less than 24 hours
      final hours = totalMinutes ~/ 60;
      final minutes = totalMinutes % 60;
      final estimate = isEstimate ? '~' : '';
      if (minutes == 0) {
        return '$estimate$hours hour${hours != 1 ? 's' : ''}';
      } else {
        return '$estimate$hours h $minutes min';
      }
      ...
      final estimate = isEstimate ? '~' : '';
      if (remainingHours == 0) {
        return '$estimate$days day${days != 1 ? 's' : ''}';
      } else {
        return '$estimate$days d $remainingHours h';
      }
    }
  }
