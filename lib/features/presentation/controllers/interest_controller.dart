...
class InterestController extends GetxController {
  ...
  Map<String, dynamic>? getSelectedInterest(
    List<Map<String, dynamic>> interests,
  ) {
    if (_selectedInterestIndex.value >= 0 &&
        _selectedInterestIndex.value < interests.length) {
  ...
  Future<void> submitInterest(List<Map<String, dynamic>> interests) async {
    try {
      ...
      // Check if any interest is selected
      if (!hasSelection) {
        Snackbars.errorSnackBar('Please select an area of interest');
        return;
      }
      ...
      final AreaOfInterestResponseModel response = await remoteSource
          .addAreaOfInterest(areaOfIntrest: selectedInterest['title']);
      ...
      Snackbars.errorSnackBar(
        'Failed to update area of interest. Please try again.',
      );
    } finally {
